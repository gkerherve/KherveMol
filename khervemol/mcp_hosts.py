"""Write KherveMol's entry into an MCP host's own config file.
...
def launch_command() -> list:
    ...
    return [sys.executable, "-m", "khervemol.mcp_server"]


def checkout_root() -> str:
    """Directory containing the ``khervemol`` package."""
...
def server_entry() -> dict:
    ...
    if not getattr(sys, "frozen", False):
        # `-m khervemol.mcp_server` only resolves if the checkout is
...
class Host:
    ...
    *path_by_platform* maps ``sys.platform`` prefixes to the config
    ...
    def connected(self) -> bool:
        """Is KherveMol already in this host's config?"""
        doc = self._read()
        if doc is None:
            return False
    ...
    def _write(self, doc: dict) -> str:
        ...
        backup = ""
        if os.path.isfile(p):
            backup = f"{p}.khervemol-{time.strftime('%Y%m%d-%H%M%S')}.bak"
            shutil.copy2(p, backup)
        tmp = f"{p}.khervemol-tmp"
    ...
    def connect(self) -> dict:
        """Add or refresh KherveMol's entry.  Returns a report."""
        if self.manual:
    ...
    def disconnect(self) -> dict:
        """Remove KherveMol's entry, leaving every other one alone."""
        if self.cli:
...
HOSTS = [
    ...
    }, note="Zed's settings file allows comments, so KherveMol will "
