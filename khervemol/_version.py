...
commit without anyone having to edit a constant. A frozen (PyInstaller)
build has no `.git` folder, so the build script writes the resolved
string to a `VERSION` file next to this one and bundles it —
`get_version()` reads that first. Outside a git checkout with no such
file we fall back to ``_FALLBACK``.
