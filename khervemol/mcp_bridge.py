"""In-app half of KherveMol's MCP support.
...
the live window.  ``mcp_server.py`` (spawned by the MCP host) and
...
will, while the tools must run on the Qt GUI thread of an
already-running window.  The socket is the seam between the two.  The
``QTcpServer`` lives on the GUI thread, so every tool call is already
executed there.
...
#: Refuse absurd payloads rather than buffering without bound.  Only a
#: rendered picture comes back large; requests are small.
_MAX_LINE = 8 * 1024 * 1024

#: Tools that only look at the document (``select_atoms`` highlights
#: atoms in the window but changes nothing in the model, and being able
#: to point at what it is describing is exactly what a read-only client
#: should be able to do; ``render_view`` may aim an offscreen camera but
#: restores the user's view).
READ_ONLY_TOOLS = frozenset({
    "get_document_info", "list_molecules", "list_crystals",
    "list_polymers", "list_reactions", "search_library",
    "get_structure", "properties", "render_view", "select_atoms",
})

#: Tools that can read or write a file the client names.  Everything
#: else stays inside the window; these reach the filesystem, so they are
...
def _names_a_path(name: str, tool_input: dict) -> bool:
    ...
    return name in FILE_TOOLS and bool(tool_input.get("path"))
...
#: talking about, or save the molecule it just built, sends you back to
#: the File menu between every step.  The connection is loopback-only,
...
class McpBridge(QObject):
    """Loopback JSON server exposing the KherveMol tools to MCP clients."""
    ...
    def _write_endpoint(self):
        ...
        try:
            ...
            # Create with 0600 from the start: the token must never be
            # world-readable, not even for the instant between creating
            # the file and tightening it.
            fd = os.open(path, os.O_WRONLY | os.O_CREAT | os.O_TRUNC, 0o600)
            with os.fdopen(fd, "w", encoding="utf-8") as fh:
                json.dump(payload, fh, indent=2)
            try:
                os.chmod(path, 0o600)   # an older file keeps its mode
            except OSError:
                pass
        except Exception:
            pass
    ...
    def _clear_endpoint(self):
        """Remove the endpoint file -- but only if it is still ours (a
        second KherveMol may have taken it over since)."""
        try:
            path = endpoint_path()
            with open(path, "r", encoding="utf-8") as fh:
                info = json.load(fh)
            if info.get("token") == self._token:
                os.remove(path)
    ...
    def _status(self) -> dict:
        ...
        return {
            ...
            "formula": mol.formula(),
            "atoms": len(mol.atoms),
            "path": getattr(self._mw, "_path", None),
        }
    ...
    def _call_tool(self, params: dict) -> dict:
        ...
        if not tool_allowed(name, self._access):
            return self._refuse(
                name, f"{name} would change the molecule, and MCP access "
        ...
        # A tool can pump the Qt event loop (a modal dialog, a nested
        # repaint), which could deliver a second request into this same
        # handler.  Two calls interleaving on one document would corrupt
        # both, so make the second wait its turn by failing.
        if self._busy:
            self._record(str(name), "busy")
            return {"error": "KherveMol is still running the previous "
