...
someone's open molecule and the auth is a per-session bearer token,
...
class McpHttpServer(QObject):
    ...
    The bridge owns the tool table and the access level; this class is
    only the protocol skin, so both transports behave identically and the
    activity log stays in one place.
    """
        ...
        if not self._authorised(req):
            self._send(sock, 401,
                       {"error": "Missing or invalid bearer token. It "
                                 "is in KherveMol's endpoint file."})
            return
        try:
    ...
    def _authorised(self, req: _Request) -> bool:
        ...
        if got.lower().startswith("bearer "):
            ...
            got = req.headers.get("x-khervemol-token", "").strip()
        import secrets
    ...
    def _initialize(self, params: dict) -> dict:
        ...
        return {
            ...
            "capabilities": {"tools": {"listChanged": False},
                             "resources": {}, "prompts": {}},
            "serverInfo": {"name": SERVER_NAME, "title": "KherveMol",
                           "version": __version__},
